...
class Program
{
    ...
    {
        Title = "Chess engine V3, Go learn C#";                       // Set the app title
        string continueGame = "";
    ...
    private static bool IsCheckmate(byte[] _board, bool isWhiteTurn)
    {
        // White turn = check for the checking of the white king

        // Not a checkmate if the king is not in check
        if (!IsKingInCheck(_board, 64, isWhiteTurn))
            return false;
        ...
        foreach (Move move in moves)
        {
            ...
            if (!IsKingInCheck(newBoard, 64, isWhiteTurn))
